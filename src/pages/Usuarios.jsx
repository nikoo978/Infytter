...
export default function Usuarios() {
  ...
  const candidatesFor = (profile) => {
    ...
    const profileDni = digits(profile.dni);
    return (data.people || [])
      .filter((person) => person.role === wantedRole && !person.archivedAt)
      .filter((person) => profile.role !== "cliente" || (profileDni && digits(person.dni) === profileDni))
    ...
    <section className="panel">
      ...
      <div className="mt-5 overflow-x-auto"><table className="w-full min-w-[1180px] text-left"><thead><tr className="table-head"><th>Cuenta PWA</th><th>Rol</th><th>Vincular desde esta cuenta</th><th>Registro</th><th className="text-right">Estado</th><th className="text-right">Cuenta</th></tr></thead><tbody className="divide-y divide-slate-100">
        {visible.map((profile) => {
          ...
          const profileDni = digits(profile.dni);
          const linkedMismatch = profile.role === "cliente" && linked && profileDni && digits(linked.dni) && profileDni !== digits(linked.dni);
            ...
            <td className="py-3">
              {profile.is_master ? <span className="text-xs font-bold text-slate-400">No requiere vínculo</span> : profile.role === "coadmin" ? <span className="text-xs font-bold text-slate-400">Coadmin · sin ficha personal</span> : <div className="min-w-[310px]">
                <select value={profile.linked_person_id || ""} onChange={(event) => changeLink(profile, event.target.value)} disabled={saving === profile.user_id} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-black outline-none disabled:opacity-50">
                  ...
                  {linkedMismatch && <option value={linked.id} disabled>⚠ Vínculo incompatible · {linked.name} · DNI {linked.dni || "—"}</option>}
                ...
                {linkedMismatch && <p className="mt-1 text-[11px] font-bold text-red-600">El DNI de la cuenta no coincide con la ficha vinculada. Desvinculá y elegí la ficha correcta.</p>}
                {linked && !linkedMismatch && <p className="mt-1 text-[11px] font-bold text-emerald-600">Vinculado a {linked.name} · {linked.role}</p>}
                {!linked && profile.role === "cliente" && !candidates.length && <p className="mt-1 text-[11px] font-bold text-amber-600">No hay una ficha Cliente libre con el mismo DNI.</p>}
                {!linked && (profile.role !== "cliente" || candidates.length > 0) && <p className="mt-1 text-[11px] font-bold text-amber-600">Elegí una ficha existente del gimnasio.</p>}
              </div>}
            </td>
            <td className="text-sm text-slate-500">{new Date(profile.created_at).toLocaleDateString("es-AR")}</td>
            <td className="text-right">{profile.is_master ? <span className="status status-bad">Protegido</span> : linkedMismatch ? <span className="status status-bad"><Unlink className="mr-1 inline size-3" /> Vínculo inválido</span> : linked ? <span className="status status-ok"><Link2 className="mr-1 inline size-3" /> Vinculado</span> : profile.role === "coadmin" ? <span className={`status ${roleTone(profile.role)}`}>{ROLE_LABELS[profile.role]}</span> : <span className="status status-warn"><Unlink className="mr-1 inline size-3" /> Pendiente</span>}</td>
